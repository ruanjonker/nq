...
handle_call({set_fun, Fun, Args}, _, State) ->
    {noreply, State#state{proc_fun = Fun, proc_args = Args}, 0};
handle_call({set_err_fun, Fun, Args}, _, State) ->
    {noreply, State#state{err_fun = Fun, err_args = Args}, 0}.

handle_cast(_, State) ->
    {noreply, State, 0}.
handle_info(timeout, #state{queue = Queue, proc_fun = Fun, proc_args = Args, err_fun = ErrFun, err_args = ErrArgs} = State) ->
    ...
    {error, not_found} ->
        ...
        CacheFun = fun(Q, M, _) ->
            ok = ?dbset("consumer_cache", Self, {Q, M, Ts, 0}) 
        end,
